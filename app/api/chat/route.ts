...
• "подросток" / "тинейджер" / "13+" → ageMax:15 (НЕ 18 — 18+ это adult-only события)
• "грудничок" / "младенец" / "до года" / "до 2 лет" → ageMax:2
• Семейный контекст без явного возраста ("семья","родителям","детьми") → НЕ ставь ageMax (естественный default ловит).

КАТЕГОРИИ — ПРЕДПОЧТИТАЙ categories[] ПЕРЕД search:
• "рисование" / "творчество" / "живопись" / "арт" / "рисовать" / "лепка" → categories:["arts"]
• "музей" / "выставка" / "экспозиция" / "галерея" → categories:["attractions","arts","education"]
• "театр" / "спектакль" / "постановка" / "балет" / "кукольный" / "пьеса" → categories:["theater"]
• "цирк" / "циркач" → categories:["theater","attractions"], search:"цирк"
• "музыка" / "концерт" / "песня" / "опера" / "филармония" / "симфония" / "оркестр" → categories:["music"]
• "танцы" / "хореография" / "балет (танец)" → categories:["theater","music"]
• "наука" / "научное" / "научно" / "технологии" / "роботы" / "робототехника" / "программирование" / "STEM" / "космос" / "космонавт" / "планетарий" / "эксперимент" / "физика" / "химия" / "биология" → categories:["education","attractions"], search:"наука"
• "природа" / "парк" / "поход" / "прогулка" / "улица" → categories:["outdoors"]
• "зоопарк" / "животные" / "звери" → categories:["outdoors","attractions"], search:"зоо"
• "спорт" / "футбол" / "плавание" / "гимнастика" / "каратэ" → categories:["outdoors"], search:"спорт"
• "книги" / "чтение" / "библиотека" / "литература" → categories:["education"], search:"книг"
• "готовить" / "кулинария" / "мастер-класс по готовке" → search:"кулинар"
• "кино" / "фильм" / "мультфильм" / "мультик" → categories:["film"]
• "праздник" / "фестиваль" / "Новый год" / "Пасха" / "Масленица" / "Хэллоуин" → search:"фестиваль"
• "день рождения" / "праздновать др" → search:"день рождения"
• "мастер-класс" / "воркшоп" / "курс" / "занятие" / "лекция" → categories:["education"]
• "экскурсия" / "тур по городу" → categories:["attractions","outdoors"], search:"экскурс"
• Широкий запрос ("куда сходить", "чем заняться") → БЕЗ фильтра категорий.
• Если пользователь упомянул "рисование и музей" → обе: categories:["arts","attractions","education"].

ОТРИЦАНИЯ ("не X", "без Y", "не хочу Z"):
• "не театр" / "не хочу спектакль" / "без театра" → excludeCategories:["theater"]
• "не музей" / "без музея" → excludeCategories:["attractions"]
• "не маркет" → search-based (нет категории market)
• "не лекция" → excludeCategories:["education"]
• Если пользователь говорит "что угодно кроме X" — используй excludeCategories.
...
Доступные поля: categories(string[]), excludeCategories(string[]), isFree(bool), ageMax(number), priceMax(number), dateFrom(YYYY-MM-DD), dateTo(YYYY-MM-DD), search(string), neighborhoods(string[]), wheelchairAccessible(bool), strollerFriendly(bool)
